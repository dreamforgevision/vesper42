...
class ScriptCollector {
  ...
  async getScriptList() {
    console.log('📋 Fetching script list from IMSDB...');
    ...
    try {
      ...
      // IMSDB has links in <p> tags within the content
      // Try multiple selectors to catch all scripts
      const selectors = [
        'p > a[href*="/Movie Scripts/"]',
        'td > a[href*="/Movie Scripts/"]', 
      ...
      selectors.forEach(selector => {
        $(selector).each((i, elem) => {
          ...
          if (href && title && !scripts.find(s => s.title === title)) {
            scripts.push({
              title,
      ...
      console.log(`✅ Found ${scripts.length} scripts available`);
      
      if (scripts.length < 50) {
        console.log('⚠️  Warning: Found fewer scripts than expected. IMSDB structure may have changed.');
      }
      
      return scripts;
      
    } catch (error) {
      console.error('❌ Error fetching script list:', error.message);
      return [];
    }
  }
  ...
  async downloadScript(scriptInfo) {
    console.log(`📥 Downloading: ${scriptInfo.title}...`);
    ...
    try {
      ...
      if (!scriptLink) {
        console.log(`⚠️  Script link not found for ${scriptInfo.title}`);
        return null;
      }
      ...
      if (!scriptText || scriptText.length < 100) {
        console.log(`⚠️  No valid script text found for ${scriptInfo.title}`);
        return null;
      }

      console.log(`✅ Downloaded ${scriptInfo.title} (${scriptText.length} characters)`);
      ...
      console.log(`❌ Error downloading ${scriptInfo.title}: ${error.message}`);
      return null;
    }
  }
  ...
  async saveScript(scriptData) {
    console.log(`💾 Saving ${scriptData.title} to database...`);
    ...
    try {
      ...
      console.log(`✅ Saved ${scriptData.title} to database!`);
      return data;
      
    } catch (error) {
      console.error(`❌ Error saving ${scriptData.title}:`, error.message);
      return null;
    }
  }

  // Main function: Collect multiple scripts
  async collectScripts(limit = 100) {
    console.log(`\n🚀 Starting script collection (limit: ${limit})...\n`);
    
    // Get list of available scripts
    const scriptList = await this.getScriptList();
    
    if (scriptList.length === 0) {
      console.log('❌ No scripts found to collect');
      return;
    }
    ...
    for (let i = 0; i < scriptsToCollect.length; i++) {
      ...
      if (existing) {
        console.log(`⏭️  ${scriptInfo.title} already in database, skipping...`);
        continue;
      }
    ...
    console.log(`\n✅ Collection complete! Successfully collected ${successCount}/${scriptsToCollect.length} scripts\n`);
  }
}
...
if (require.main === module) {
  ...
  collector.collectScripts(100)
    .then(() => {
      console.log('🎉 Script collection finished!');
      process.exit(0);
    })
    .catch(error => {
